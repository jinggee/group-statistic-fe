<template>
  <div class="area-filter">
    ...
    <div  v-if="data && data.length > 0" class="flex area-list">
      <div v-for="item in displayedProvince" :key="item.name"
        class="area-item"
        :class="{'selected': item === selectedProvince}"
        @click="onClickItem(item, 1)">
        {{`${item.name}(${item.value})`}}
      </div>
      <div class="area-item expand-area"
        v-if="showExpand"
        @click="toogleExpand">
    ...
    <div class="flex area-list" v-if="showCitys">
        <div v-for="item in selectedProvince?.children" :key="item.name"
            class="area-item"
...
<script lang="ts">
import {
    defineComponent, PropType, Ref, unref, ref, computed,
} from 'vue'
import {
    Col, Row, Empty, Button,
} from 'vant'
