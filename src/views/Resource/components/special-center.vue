...
<template>
    <div>
        <section-view title="省级重点专科">
            ...
            <umcare-table :columns="columns" :data="mainSpecialDept"></umcare-table>
        </section-view>
        <section-view title="省级重点建设专科">
            <template #head-tool>
                <span class="hospital-sum">{{ `${mainBuildDept?.length ?? 0}家` }}</span>
            </template>
            <umcare-table :columns="columns" :data="mainBuildDept"></umcare-table>
        </section-view>
        <section-view title="特色专科" class="p-b-20">
            <template #head-tool>
                <span class="hospital-sum">{{ `${characteristicDept?.length ?? 0}家` }}</span>
            </template>
            <umcare-table :columns="columns" :data="characteristicDept"></umcare-table>
        </section-view>
    </div>
</template>
